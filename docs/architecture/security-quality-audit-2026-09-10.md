...
| AQ-06 | Médio / Confirmado | Convites administrativos expirados não possuíam ação de reenvio ou cancelamento, e uma nova tentativa com o mesmo e-mail era recusada pelo diretório de autenticação. | Um membro pendente podia ficar bloqueado sem caminho operacional seguro para receber outro link. | **Corrigido**: convites pendentes podem ser reenviados ou cancelados; o reenvio invalida o usuário pendente anterior, preserva papel/e-mail obtidos no servidor e cria um convite novo. Exclusões no Auth são verificadas e a associação é compensada se o diretório recusar a operação. |
